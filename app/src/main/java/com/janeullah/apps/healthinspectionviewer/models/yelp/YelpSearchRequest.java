    ...
    public YelpMatch scoreMatch(Business yelpBusinessObject) {
        ...
        if (matchesCityStateZip(yelpBusinessObject)) {
            ...
            int longestStringLength =
                    Math.max(
                            getStringLength(restaurantMetadata.getAddressLine1()),
                            getStringLength(yelpLocation.getAddress1()));
            int levenshteinDistance =
                    StringUtils.getLevenshteinDistance(
                            trimString(yelpLocation.getAddress1()),
                            trimString(restaurantMetadata.getAddressLine1()));
            addressMatchScore = 1.0 - ((levenshteinDistance * 1.0) / longestStringLength);

            longestStringLength =
                    Math.max(
                            getStringLength(restaurant.name),
                            getStringLength(yelpBusinessObject.getName()));
            levenshteinDistance =
                    StringUtils.getLevenshteinDistance(
                            trimString(yelpBusinessObject.getName()), trimString(restaurant.name));
