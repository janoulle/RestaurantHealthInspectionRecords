    ...
    public static final Ordering<YelpMatch> YELP_MATCH_ORDERING =
            new Ordering<YelpMatch>() {
                ...
                public int compare(YelpMatch left, YelpMatch right) {
                    return PAIR_MATCH_COMPARATOR.compare(left.nameAndAddressScorePair,right.nameAndAddressScorePair);
                }
            };

    public static final Comparator<Pair<Double,Double>> PAIR_MATCH_COMPARATOR = new Comparator<Pair<Double, Double>>() {
        @Override
        public int compare(Pair<Double, Double> left, Pair<Double, Double> right) {
            if (Doubles.compare(left.first,right.first) > 0 && Doubles.compare(left.second,right.second) > 0){
                return 1;
            }else if (Doubles.compare(left.first,right.first) < 0 && Doubles.compare(left.second,right.second) < 0){
                return -1;
            }else if (Doubles.compare(left.first,right.first) == 0 && Doubles.compare(left.second,right.second) == 0){
                return 0;
            }else{
