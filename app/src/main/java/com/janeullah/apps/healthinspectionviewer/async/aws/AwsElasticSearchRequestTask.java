 ...
 * @date 9/29/2017.
 */
public class AwsElasticSearchRequestTask
        extends AsyncTask<AwsElasticSearchRequest, Integer, ElasticSearchResponse>
    ...
    protected ElasticSearchResponse doInBackground(
            AwsElasticSearchRequest... awsElasticSearchRequests) {
        try {
            Log.i(TAG, "Initiated background processing...");
            AwsElasticSearchService elasticSearchService =
                    RetrofitConfiguration.AWS_ELASTIC_SEARCH_SERVICE;
            Call<ElasticSearchResponse> searchRequest =
                    elasticSearchService.findRestaurantsByName(
                            awsElasticSearchRequests[0].getHeaders(),
                            awsElasticSearchRequests[0].getPayload());
