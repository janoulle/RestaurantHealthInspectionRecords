...
public class EventLoggingUtils {
    ...
    public static void logSelectionEvent(
            String key,
            String value,
            String tag,
        ...
        bundle.putString(
                FirebaseAnalytics.Param.ITEM_ID, UUIDInitializer.getInstance(activity).getUUID());
        bundle.putString(FirebaseAnalytics.Param.ITEM_NAME, key);
        bundle.putString(FirebaseAnalytics.Param.VALUE, value);
        bundle.putLong(
                FirebaseAnalytics.Param.START_DATE, Calendar.getInstance().getTimeInMillis());
        firebaseAnalytics.logEvent(FirebaseAnalytics.Event.SELECT_CONTENT, bundle);
        logSelectionEventWithFabric(key, value, tag, activity);
    }

    private static void logSelectionEventWithFabric(
            String key, String value, String tag, BaseActivity activity) {
        Answers.getInstance()
                .logCustom(
                        new CustomEvent("Selection Made")
                                ...
                                .putCustomAttribute(
                                        ActionParameters.UUID.text(),
                                        UUIDInitializer.getInstance(activity).getUUID())
                                .putCustomAttribute(
    ...
    public static void logViewEvent(
            String tag, FirebaseAnalytics firebaseAnalytics, BaseActivity activity) {
        Bundle bundle = new Bundle();
        bundle.putString(FirebaseAnalytics.Param.LOCATION, tag);
        bundle.putString(
                FirebaseAnalytics.Param.ITEM_ID, UUIDInitializer.getInstance(activity).getUUID());
    ...
    private static void logViewEventsWithFabric(
            String tag, String contentType, BaseActivity activity) {
        Answers.getInstance()
                .logContentView(
