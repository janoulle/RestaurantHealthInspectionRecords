 ...
 * demonstrating how to sign 'chunked' uploads
 */
public class AWS4SignerForChunkedUpload extends AWS4SignerBase {
     ...
     * included in the Authorization header. For subsequent chunks it contains the computed
     * signature of the prior chunk.
     */
    ...
    public byte[] constructSignedChunk(int userDataLen, byte[] userData) {
        ...
        try {
            ...
            byte[] trailer = CLRF.getBytes(Charsets.UTF_8);
            ...
            System.arraycopy(
                    trailer, 0, signedChunk, header.length + dataToChunk.length, trailer.length);
