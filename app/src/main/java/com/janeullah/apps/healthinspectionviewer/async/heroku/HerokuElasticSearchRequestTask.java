 ...
 * @date 10/21/2017.
 */
public class HerokuElasticSearchRequestTask
        extends AsyncTask<HerokuElasticSearchRequest, Integer, ElasticSearchResponse>
        implements AsyncElasticSearchTaskable {
    private static final String TAG = "AwsSearchTask";
    private static final ObjectMapper mapper = new ObjectMapper();
    private ElasticSearchTaskListener elasticSearchTaskListener;

    @Override
    protected ElasticSearchResponse doInBackground(
            HerokuElasticSearchRequest... herokuElasticSearchRequests) {
        try {
            Log.i(TAG, "Initiated background processing...");
            HerokuElasticSearchService elasticSearchService =
                    RetrofitConfiguration.HEROKU_ELASTIC_SEARCH_SERVICE;
            Call<ElasticSearchResponse> searchRequest =
                    elasticSearchService.findRestaurantsByName(
                            herokuElasticSearchRequests[0].getHeaders(),
                            herokuElasticSearchRequests[0].getPayload());
            ...
            if (response.isSuccessful()) {
                ...
                HerokuAppSleepingResponse herokuAppSleepingResponse =
                        tryGetHerokuAppSleepingResponse(response);
                if (herokuAppSleepingResponse != null) {
                    Log.d(
                            TAG,
                            "Heroku dyno is asleep. Full response: " + herokuAppSleepingResponse);
                }
            }
